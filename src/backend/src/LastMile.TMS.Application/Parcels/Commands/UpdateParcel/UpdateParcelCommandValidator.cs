...
public sealed class UpdateParcelCommandValidator : AbstractValidator<UpdateParcelCommand>
{
    public UpdateParcelCommandValidator()
    {
        ...
        RuleFor(x => x.Dto.RecipientAddress.ContactName)
            .MaximumLength(200).WithMessage("Recipient contact name must not exceed 200 characters.");

        RuleFor(x => x.Dto.RecipientAddress.CompanyName)
            .MaximumLength(200).WithMessage("Recipient company name must not exceed 200 characters.");

        RuleFor(x => x.Dto.RecipientAddress.Phone)
            .MaximumLength(50).WithMessage("Recipient phone must not exceed 50 characters.");

        RuleFor(x => x.Dto.RecipientAddress.Email)
            .MaximumLength(256).WithMessage("Recipient email must not exceed 256 characters.");
    }
}
